...
--
-- Cubre:
--   - G1 ⭐ Treatment Card visual (ortho_treatment_cards + 3 hijas)
...
-- OrthodonticDiagnosis: añadir patrón skeletal categórico (Sección B).
ALTER TABLE "orthodontic_diagnoses"
  ADD COLUMN IF NOT EXISTS "skeletalPattern" "OrthoSkeletalPattern";

-- OrthodonticTreatmentPlan: añadir prescripción G4 detallada.
...
-- 3.1 ortho_wire_steps (G3 wire sequencing)
...
-- 3.2 ortho_treatment_cards (G1 ⭐ visual SOAP)
...
-- 3.3 ortho_card_elastics (hijo de treatment_cards)
...
-- 3.4 ortho_card_ipr_points (hijo de treatment_cards)
...
-- 3.5 ortho_card_broken_brackets (hijo de treatment_cards)
...
-- 3.6 ortho_tads (G10 mini-implants ortodónticos)
...
-- 3.7 ortho_aux_mechanics (G10 expander/distalizer 1:1 con plan)
...
-- 3.8 ortho_phase_transitions (audit trail de avance de fase)
