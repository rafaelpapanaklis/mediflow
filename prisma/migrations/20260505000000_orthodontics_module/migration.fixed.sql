-- ═══════════════════════════════════════════════════════════════════
-- Orthodontics module — schema (foundation) — VERSIÓN FIXED
--
-- ⚠️  CAMBIO RESPECTO A migration.sql:
--     El cuerpo de la función `ortho_recalc_payment_plan_status()` usa
--     ahora dollar-quote ETIQUETADO (`$ortho_recalc$ … $ortho_recalc$`)
--     en lugar de `$$ … $$`. Razón: el SQL Editor de Supabase parsea
--     inconsistentemente el cuerpo de la función con `$$` después de
--     los 71 bloques `DO $$ ... END $$;` previos en el mismo archivo,
--     fragmenta el body y genera el error
--       `42P01: relation "v_paid" does not exist`
--     porque las variables PL/pgSQL se interpretan como referencias a
--     tablas inexistentes en los chunks fragmentados.
--
--     Postgres acepta `$ortho_recalc$` idénticamente a `$$` — el cambio
--     es solo de parser-friendliness.
--
-- ⚠️  ANTES de aplicar este script:
--     1. Si la migración original `migration.sql` se aplicó parcialmente,
--        corre `cleanup.sql` (mismo directorio) primero.
--     2. Verifica con la query final del cleanup que no queda residuo.
--     3. Aplica este `migration.fixed.sql` completo en el SQL Editor.
--
-- CONTEXTO
...
RETURNS TRIGGER AS $ortho_recalc$
DECLARE
  v_paid       DECIMAL(10,2);
