    ...
    <div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        {[
          ...
          { icon:"🏥", label:"Clínica",            value: user.clinic.plan,          sub:`Trial activo`,                                                         bg:"bg-violet-50"  },
        ].map(k => (
          <div key={k.label} className="rounded-xl border border-border bg-white p-5 shadow-card">
            ...
            <div className="text-2xl font-extrabold text-foreground mb-0.5">{k.value}</div>
            <div className="text-xs font-semibold text-muted-foreground">{k.label}</div>
            {k.sub && <div className="text-xs text-muted-foreground/70 mt-0.5">{k.sub}</div>}
          </div>
        ))}
      </div>
      ...
      <div className="grid lg:grid-cols-3 gap-5 mb-5">
        ...
        <div className="lg:col-span-2 rounded-xl border border-border bg-white p-5 shadow-card">
          ...
          {todayAppts.length === 0 ? (
            <div className="text-center py-8 text-muted-foreground text-sm">No hay citas para hoy.<br/>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-border bg-muted/30">
              ...
              <th className="text-left px-5 py-3 text-xs font-semibold text-muted-foreground uppercase tracking-wide hidden sm:table-cell">Teléfono</th>
              <th className="text-left px-5 py-3 text-xs font-semibold text-muted-foreground uppercase tracking-wide hidden md:table-cell">Registro</th>
            </tr>
          </thead>
          <tbody>
            {recentPatients.length === 0 ? (
              <tr><td colSpan={3} className="px-5 py-10 text-center text-muted-foreground text-sm">
                ...
                <Link href="/dashboard/patients" className="text-brand-600 hover:underline">Agrega el primero →</Link>
              </td></tr>
