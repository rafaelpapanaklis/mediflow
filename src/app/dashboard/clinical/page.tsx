...
export const metadata: Metadata = { title: "Expediente Clínico — MediFlow" };
...
export default async function ClinicalPage({ searchParams }: { searchParams: { patientId?: string } }) {
  ...
  const patients = await prisma.patient.findMany({
    ...
    select:  { id: true, firstName: true, lastName: true, patientNumber: true, gender: true, dob: true },
  ...
  if (searchParams.patientId) {
    selectedPatient = await prisma.patient.findFirst({
      where: { id: searchParams.patientId, clinicId: user.clinicId },
    });
    if (selectedPatient) {
      records = await prisma.medicalRecord.findMany({
        where:   { patientId: searchParams.patientId, clinicId: user.clinicId },
