...
export default async function AppointmentsPage() {
  ...
  const [todayAppts, patients, doctors] = await Promise.all([
    prisma.appointment.findMany({ where: { clinicId: user.clinicId, date: { gte: today, lte: todayEnd } }, include: { patient: true, doctor: true }, orderBy: { startTime: "asc" } }),
    ...
    prisma.user.findMany({ where: { clinicId: user.clinicId, isActive: true }, select: { id: true, firstName: true, lastName: true, role: true } }),
  ]);
