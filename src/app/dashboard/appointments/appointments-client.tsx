...
export function AppointmentsClient({ todayAppts: initial, patients, doctors, currentUserId }: Props) {
  ...
  const [form, setForm] = useState({
    ...
    startTime: "09:00", endTime: "09:30", durationMins: 30, notes: "",
  });
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-muted-foreground">{appts.length} citas hoy</p>
        </div>
        ...
        <div className="rounded-xl border border-brand-200 bg-brand-50 p-5 mb-5 animate-fade-up">
          ...
          <form onSubmit={createAppointment} className="grid grid-cols-2 lg:grid-cols-3 gap-3">
            ...
            <div className="space-y-1">
              ...
              <input type="date" className="flex h-10 w-full rounded-lg border border-border bg-white px-3 text-sm focus:outline-none focus:ring-2 focus:ring-brand-600/20"
                value={form.date} onChange={e => set("date", e.target.value)} />
            </div>
            <div className="space-y-1">
              <label className="text-xs font-bold text-muted-foreground">Inicio</label>
              <input type="time" className="flex h-10 w-full rounded-lg border border-border bg-white px-3 text-sm focus:outline-none focus:ring-2 focus:ring-brand-600/20"
            ...
            <div className="col-span-2 lg:col-span-3 space-y-1">
              ...
              <input className="flex h-10 w-full rounded-lg border border-border bg-white px-3 text-sm focus:outline-none"
                placeholder="Motivo de consulta…" value={form.notes} onChange={e => set("notes", e.target.value)} />
            </div>
            <div className="col-span-2 lg:col-span-3 flex gap-2 pt-1">
      ...
      <div className="space-y-3">
        {appts.length === 0 ? (
          <div className="rounded-xl border border-border bg-white p-16 text-center shadow-card">
            ...
            <Button size="sm" className="mt-3" onClick={() => setShowNew(true)}>Agendar primera cita</Button>
          </div>
        ) : appts.map(appt => {
          ...
          return (
            <div key={appt.id} className="rounded-xl border border-border bg-white p-4 shadow-card flex items-center gap-4">
              <div className="flex-shrink-0 text-center w-14">
                <div className="text-sm font-extrabold text-foreground">{appt.startTime}</div>
              ...
              <div className="flex gap-1.5 flex-shrink-0">
                {appt.status === "PENDING" && (
                  <button onClick={() => changeStatus(appt.id, "CONFIRMED")}
                    className="p-1.5 rounded-lg hover:bg-emerald-50 text-emerald-600 transition-colors" title="Confirmar">
                ...
                {!["CANCELLED", "COMPLETED", "NO_SHOW"].includes(appt.status) && (
                  <>
                    <button onClick={() => changeStatus(appt.id, "COMPLETED")}
                      className="p-1.5 rounded-lg hover:bg-blue-50 text-blue-600 transition-colors" title="Completar">
                      <CheckCircle className="w-4 h-4" />
                    </button>
                    <button onClick={() => changeStatus(appt.id, "CANCELLED")}
                      className="p-1.5 rounded-lg hover:bg-rose-50 text-rose-600 transition-colors" title="Cancelar">
