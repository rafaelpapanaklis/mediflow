    ...
    <div className="p-4 lg:p-8 max-w-7xl mx-auto space-y-6">
      ...
      <div className="relative">
        ...
        <input
          ...
          className="w-full pl-10 pr-4 py-2.5 bg-white dark:bg-slate-900 border border-border rounded-xl text-sm text-slate-900 dark:text-white placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-brand-500/40 focus:border-brand-500"
        />
      </div>

      {/* Empty state */}
      {filtered.length === 0 ? (
        <div className="bg-white dark:bg-slate-900 border border-border rounded-2xl shadow-card p-12 text-center">
        ...
        <div className="space-y-6">
          {grouped.map(([category, items]) => (
            <div
              key={category}
              className="bg-white dark:bg-slate-900 border border-border rounded-2xl shadow-card overflow-hidden"
            >
              <div className="px-5 py-3 border-b border-border bg-slate-50 dark:bg-slate-800/50">
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead>
                    <tr className="text-left text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400 border-b border-border">
                  ...
                  <tbody>
                    {items.map((p) => (
                      <tr
                        key={p.id}
                        className={`border-b border-border last:border-0 hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-colors ${
                        ...
                        <td className="px-5 py-3">
                          <div className="flex items-center justify-end gap-1">
                            <button
                              onClick={() => openEdit(p)}
                              title="Editar"
                              className="p-1.5 rounded-lg text-slate-500 hover:text-brand-600 hover:bg-brand-50 dark:hover:bg-brand-900/20 transition-colors"
                            >
      ...
      {modalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div
            ...
            onClick={closeModal}
          />
          <div className="relative bg-white dark:bg-slate-900 border border-border rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2 bg-white dark:bg-slate-800 border border-border rounded-lg text-sm text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-brand-500/40 focus:border-brand-500"
                />
              </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={3}
                  placeholder="Opcional"
                  className="w-full px-3 py-2 bg-white dark:bg-slate-800 border border-border rounded-lg text-sm text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-brand-500/40 focus:border-brand-500 resize-none"
                />
              </div>
              ...
              <div className="flex items-center justify-end gap-2 pt-4 border-t border-border">
                <button
                  ...
                  onClick={closeModal}
                  disabled={saving}
                  className="px-4 py-2 text-sm font-semibold text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg transition-colors disabled:opacity-50"
                >
                  Cancelar
                </button>
                <button
