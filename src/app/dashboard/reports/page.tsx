...
export default async function ReportsPage() {
  ...
  const monthlyData = await Promise.all(months.map(async ({ year, month }) => {
    ...
    const label = start.toLocaleDateString("es-MX", { month: "short", year: "2-digit" });
    return { label, revenue: revenue._sum.paid ?? 0, patients, appointments };
  }));

  const [topTypes, byStatus] = await Promise.all([
    prisma.appointment.groupBy({
      by: ["type"], where: { clinicId }, _count: { id: true },
      orderBy: { _count: { id: "desc" } }, take: 6,
    }),
    prisma.appointment.groupBy({
      by: ["status"], where: { clinicId }, _count: { id: true },
    }),
  ]);
