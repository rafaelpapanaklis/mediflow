...
import { LabStatusLogout } from "@/components/laboratorios/lab-status-logout";
...
export default async function LabPendingPage() {
  const ctx = await getDentalLabContext();
  if (!ctx) redirect("/laboratorios/login");
  if (ctx.status === "APPROVED") redirect("/laboratorios/inicio");

  const rejectedReason = ctx.lab.rejectedReason ?? null;
  ...
  if (ctx.status === "REJECTED") {
    ...
    copy = {
      ...
      body:
        "El acceso de tu cuenta de laboratorio ha sido suspendido. Contacta al equipo de DaleControl para más información sobre cómo restablecerlo.",
      tone: "danger",
    };
  } else {
    copy = {
      ...
      body:
        "El equipo de DaleControl revisará tu solicitud y recibirás acceso al panel de laboratorios en cuanto sea aprobada. Te notificaremos por correo cuando esté lista.",
      tone: "neutral",
    };
  }
