...
import { slugifySupplier } from "@/lib/suppliers/types";
...
export async function POST(req: Request) {
  ...
  const paymentMethods = Array.isArray(body?.paymentMethods)
    ? body.paymentMethods.filter((p: unknown): p is string => typeof p === "string")
    : [];
  ...
  const { data: created, error: createError } = await getAdminClient().auth.admin.createUser({
    ...
    password,
    email_confirm: true,
    user_metadata: { firstName, lastName, businessName },
  });
  ...
  // d) Slug único.
  let slug = slugifySupplier(businessName);
  for (let attempt = 0; attempt < 5; attempt++) {
    const existing = await prisma.supplier.findUnique({ where: { slug } });
    if (!existing) break;
    const suffix = Math.random().toString(36).slice(2, 6);
    slug = `${slugifySupplier(businessName)}-${suffix}`;
  }

  // e) Crear Supplier + SupplierUser en una transacción.
  try {
