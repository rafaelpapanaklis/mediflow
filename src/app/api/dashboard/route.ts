...
export async function GET() {
  try {
    ...
    const lastPrevMonth = new Date(now.getFullYear(), now.getMonth(), 0); lastPrevMonth.setHours(23,59,59,999);
    const nextSevenDays = new Date(now); nextSevenDays.setDate(nextSevenDays.getDate()+7);

    const [todayAppts, monthAppts, prevMonthAppts, monthPatients, prevMonthPatients,
           monthRevenue, prevMonthRevenue, pendingInvoices, unconfirmedAppts,
           nextWeekAppts, lowInventory, recentPatients, doctorStats] = await Promise.all([
      ...
      prisma.appointment.findMany({ where:{ clinicId, date:{gt:todayEnd,lte:nextSevenDays}, status:{not:"CANCELLED"}},
    ...
    const totalSlots = activeDoctor * workingDays * 8;
    const occupancy = totalSlots > 0 ? Math.min(100, Math.round((monthAppts/totalSlots)*100)) : 0;
    ...
    return NextResponse.json({
      ...
      pendingBalance: pendingInvoices._sum.balance ?? 0, pendingInvoices: pendingInvoices._count,
