  ...
  try {
    const result = await provider.generateInvoice({
      ...
      conceptos: [{
        ...
        cantidad: 1,
        claveUnidad: "E48",
        descripcion: `Suscripción DaleControl ${clinic.plan} · ${payment.periodStart.toISOString().slice(0,10)} → ${payment.periodEnd.toISOString().slice(0,10)}`,
    ...
    await prisma.cfdiRecord.create({
      data: {
        ...
        conceptos: [{ descripcion: "Suscripción DaleControl", importe: payment.amount }],
