...
async function resolveAfter(after: string | null, threadId: string): Promise<Date | null> {
  ...
  const msg = await prisma.dentalLabChatMessage.findUnique({
    where: { id: after },
...
export async function GET(req: Request, { params }: { params: { threadId: string } }) {
  ...
  let rows;
  if (afterDate) {
    rows = await prisma.dentalLabChatMessage.findMany({
      where: { threadId: thread.id, createdAt: { gt: afterDate } },
      orderBy: { createdAt: "asc" },
      select: MESSAGE_SELECT,
    });
  } else {
    const recent = await prisma.dentalLabChatMessage.findMany({
  ...
  if (!afterParam) {
    await prisma.dentalLabChatThread.update({
      where: { id: thread.id },
      data: caller.side === "CLINIC" ? { clinicUnread: 0 } : { labUnread: 0 },
    });
  }
...
export async function POST(req: Request, { params }: { params: { threadId: string } }) {
  ...
  const sender: DentalLabChatSender = caller.side === "CLINIC" ? "CLINIC" : "LAB";

  const message = await prisma.$transaction(async (tx) => {
    const created = await tx.dentalLabChatMessage.create({
      data: { threadId: thread.id, sender, senderId: caller.senderId, body: text },
      select: MESSAGE_SELECT,
    });
    await tx.dentalLabChatThread.update({
      where: { id: thread.id },
      data: {
        lastMessageAt: created.createdAt,
        ...(sender === "CLINIC"
          ? { labUnread: { increment: 1 }, clinicUnread: 0 }
          : { clinicUnread: { increment: 1 }, labUnread: 0 }),
      },
    });
    return created;
  });
