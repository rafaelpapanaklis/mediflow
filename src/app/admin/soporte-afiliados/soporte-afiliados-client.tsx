...
// ═══════════════════════════════════════════════════════════════════════════
// /admin/soporte-afiliados — bandeja global de tickets de AFILIADOS.
//
// Espejo de /admin/soporte (clínicas): KpiCards + segment de estado (pseudo-
// valor "OPEN" = abiertos) + selects de categoría/prioridad + búsqueda server
// (folio/asunto, debounce) + filtro de afiliado CLIENT-SIDE + tabla .table-new
// con fila clicable.
//
// Se duplica la pantalla en vez de generalizar la de clínicas: aquella está en
// producción, sus tipos (SUPPORT_*) y sus columnas (clínica, adjuntos) son
// otros, y tocarla para meter afiliados arriesgaría soporte de clínicas.
//
// API: GET /api/admin/affiliate-support/tickets?status=&category=&priority=&q=&metrics=1
...
import { AlertTriangle, Clock, Handshake, Hourglass, Inbox, LifeBuoy, Search, Star } from "lucide-react";
...
import type {
  AdminAffiliateTicketSummary,
  AffiliateSupportAdminMetrics,
} from "@/lib/affiliate-support/types";
...
const STATUS_SEGMENTS: { value: string; label: string }[] = [
  ...
  ...AFFILIATE_SUPPORT_STATUSES.map((s) => ({
    value: s as string,
    label: AFFILIATE_SUPPORT_STATUS_LABELS_ADMIN[s] ?? (s as string),
  })),
];
...
// Columnas: Folio · Afiliado · Asunto · Categoría · Prioridad · Estado · Espera · Últ. actividad · ★
const COL_COUNT = 9;
const SKEL_WIDTHS = [64, 130, 220, 80, 56, 110, 48, 64, 24];
const HIDDEN_MD_COLS = [3, 8]; // Categoría y ★ se ocultan en pantallas angostas

export function AdminSoporteAfiliadosClient() {
  const router = useRouter();

  // Filtros server-side (re-fetchean) + filtro de afiliado client-side.
  const [status, setStatus] = useState("OPEN"); // default al entrar: abiertos
  const [category, setCategory] = useState("");
  const [priority, setPriority] = useState("");
  const [q, setQ] = useState("");
  const [appliedQ, setAppliedQ] = useState("");
  const [affiliateQ, setAffiliateQ] = useState("");

  const [tickets, setTickets] = useState<AdminAffiliateTicketSummary[]>([]);
  const [metrics, setMetrics] = useState<AffiliateSupportAdminMetrics | null>(null);
  const [sqlPending, setSqlPending] = useState<string | null>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const res = await fetch(`/api/admin/affiliate-support/tickets?${params.toString()}`, {
        ...
        // El API responde 200 con sqlPending cuando faltan las tablas: bandeja
        // vacía + aviso accionable, nunca una pantalla de error.
        setSqlPending(
          data?.sqlPending
            ? data.sqlPendingMessage ?? "Falta aplicar el SQL de soporte de afiliados en Supabase."
            : null,
        );
        setError(false);
      } catch (err: any) {
  ...
  // Filtro de afiliado: client-side por affiliateName contains.
  const visible = useMemo(() => {
    const aq = affiliateQ.trim().toLowerCase();
    if (!aq) return tickets;
    return tickets.filter((t) => (t.affiliateName || "").toLowerCase().includes(aq));
  }, [tickets, affiliateQ]);

  const hasExtraFilters = !!(category || priority || appliedQ.trim() || affiliateQ.trim());
  ...
  function goToTicket(id: string) {
    router.push(`/admin/soporte-afiliados/${id}`);
  }
    ...
    <div style={{ maxWidth: 1280, margin: "0 auto", padding: "32px 24px" }}>
      ...
      <div style={{ marginBottom: 22 }}>
        ...
        <p style={{ color: "var(--text-3)", fontSize: 13, margin: "4px 0 0" }}>
          Tickets de los afiliados: comisiones, pagos y material de venta
        </p>
      </div>

      {/* Aviso de SQL pendiente (tablas affiliate_support_* sin crear) */}
      {sqlPending && (
        <div
          style={{
            display: "flex",
      ...
      <div className="grid grid-cols-2 xl:grid-cols-5" style={{ gap: 14, marginBottom: 20 }}>
        ...
        />
        {/* Mismo número que el badge del sidebar (countAffiliateSupportPendingReply). */}
        <KpiCard
          label="Sin responder"
      ...
      <div style={{ display: "flex", gap: 12, marginBottom: 16, flexWrap: "wrap", alignItems: "center" }}>
        ...
        >
          ...
          {AFFILIATE_SUPPORT_CATEGORIES.map((c) => (
            <option key={c} value={c}>
              {AFFILIATE_SUPPORT_CATEGORY_LABELS[c] ?? c}
            </option>
          ))}
        </select>
        ...
        <div className="search-field" style={{ flex: "1 1 160px", maxWidth: 220 }}>
          ...
          <input
            ...
            value={affiliateQ}
            onChange={(e) => setAffiliateQ(e.target.value)}
        ...
        {error && !loading ? (
          ...
          <div style={{ padding: "56px 24px", textAlign: "center" }}>
            ...
            <div style={{ fontSize: 12.5, color: "var(--text-3)", marginBottom: status === "OPEN" ? 16 : 0 }}>
              {status === "OPEN"
                ? "La bandeja está al día. Cuando un afiliado abra un ticket aparecerá aquí."
                : "Cuando un afiliado escriba a soporte, su ticket aparecerá aquí."}
            </div>
            {status === "OPEN" && (
                  ...
                  Array.from({ length: 5 }).map((_, i) => (
                    <tr key={`skel-${i}`}>
                      {SKEL_WIDTHS.map((w, j) => (
                        <td
                          key={j}
                          className={HIDDEN_MD_COLS.indexOf(j) !== -1 ? "hidden md:table-cell" : undefined}
                  ...
                  <>
                    {visible.map((t) => (
                      <tr
                        ...
                        onKeyDown={(e) => {
                          if (e.key === "Enter") goToTicket(t.id);
                        }}
                        tabIndex={0}
                        style={{ cursor: "pointer" }}
                      >
                        {/* Acento a la izquierda cuando el afiliado espera respuesta */}
                        <td style={{ borderLeft: `3px solid ${t.needsReply ? "var(--danger)" : "transparent"}` }}>
                          <span className="mono" style={{ fontSize: 12, color: "var(--text-1)", fontWeight: 500, whiteSpace: "nowrap" }}>
                            {t.folioLabel || formatAffiliateFolio(t.folio)}
                          </span>
                        </td>
                        <td>
                        ...
                        <td className="hidden md:table-cell" style={{ fontSize: 12, color: "var(--text-2)", whiteSpace: "nowrap" }}>
                          {AFFILIATE_SUPPORT_CATEGORY_LABELS[t.category] ?? t.category}
                        </td>
                        <td>
                          <span style={{ fontSize: 12, fontWeight: t.priority === "URGENTE" ? 600 : 500, color: priorityColor(t.priority), whiteSpace: "nowrap" }}>
                            {AFFILIATE_SUPPORT_PRIORITY_LABELS[t.priority] ?? t.priority}
                          </span>
                        </td>
                        <td>
                          <BadgeNew tone={STATUS_TONES[t.status] ?? "neutral"} dot>
                            {AFFILIATE_SUPPORT_STATUS_LABELS_ADMIN[t.status] ?? t.status}
                          </BadgeNew>
                        </td>
                        <td>
                          {t.needsReply ? (
