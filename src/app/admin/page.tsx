          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-slate-700">
                {["Clínica","Especialidad","Plan","Pacientes","Registro","Estado"].map(h => (
                ...
                return (
                  <tr key={clinic.id} className="border-b border-slate-800 hover:bg-slate-800/50 transition-colors">
                    <td className="px-4 py-3">
                      ...
                      <div className="text-[10px] text-slate-500 font-mono">{clinic.slug}</div>
                    </td>
                    <td className="px-4 py-3 text-slate-300">{clinic.specialty}</td>
                    <td className="px-4 py-3">
                      <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full border ${
                        clinic.plan === "CLINIC" ? "bg-violet-900/50 text-violet-400 border-violet-700" :
                        clinic.plan === "PRO"    ? "bg-brand-900/50 text-brand-400 border-brand-700" :
