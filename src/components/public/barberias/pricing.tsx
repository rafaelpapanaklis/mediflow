...
import { OficioNavaja, OficioTijeras } from "./oficio";
import { rd } from "./problem";

/**
 * El tablero de precios: los tres planes, tal cual la tabla — precio,
 * primer mes y anual (solo si la fila los trae), límites y features. El
 * texto de cada feature vive en el diccionario (pricing.features.<key>);
 * si una llave nueva del catálogo todavía no tiene texto, se pinta la
 * etiqueta del catálogo y no la llave.
 *
 * Lo que se lee de un vistazo: el nombre del plan en letrero, el precio
 * grande en la misma línea base en las tres tarjetas, y el destacado con
 * la cinta del poste arriba.
 */
...
export function BarberPricing({ t, cards }: { t: TFunction; cards: BarberPlanCardVM[] }) {
  return (
    <section className="dcbl-section dcbl-pricing dcbl-grain" id={BARBER_LANDING_ANCHORS.pricing}>
      <div className="dcbl-wrap">
        ...
        <div className="dcbl-nofees" data-reveal="">
          <h3 className="dcbl-nofees__title">
            <OficioNavaja size={22} />
            {t("pricing.noFees.title")}
          </h3>
