...
/**
 * Precios en MXN. Mensual confirmado; anual es placeholder ("2 meses gratis").
 * EDITABLE — ajusta montos y features aquí. // TODO confirmar precios anuales.
 */
const PRICING = {
  basic: {
    name: "Basic", desc: "Para ordenar tu clínica desde el día uno", popular: false,
    monthly: 499, annual: 4990, plan: "basic",
    feats: [
  ...
  pro: {
    ...
    monthly: 999, annual: 9990, plan: "pro",
    feats: [
  ...
  clinic: {
    ...
    monthly: 1999, annual: 19990, plan: "clinic",
    feats: [
        ...
        <div className="mfh-head mfh-center mfh-reveal">
          ...
          <div className="mfh-toggle" role="group" aria-label="Periodo de facturación">
            ...
            <button type="button" className="mfh-toggle__btn" data-active={annual} onClick={() => setAnnual(true)} aria-pressed={annual}>
              Anual <span className="mfh-toggle__save">2 meses gratis</span>
            </button>
          </div>
        </div>
          ...
          {ORDER.map((key) => {
            ...
            return (
              <div key={key} className={`mfh-plan${p.popular ? " mfh-plan--pop" : ""} mfh-reveal`}>
                {p.popular && <span className="mfh-plan__pop">El más elegido</span>}
                <div>
                ...
                <div>
                  ...
                  <div className="mfh-plan__note" style={{ textAlign: "left", marginTop: 6 }}>
                    {annual ? "Facturado al año · 2 meses gratis" : "Sin permanencia · cancela cuando quieras"}
                  </div>
                </div>
