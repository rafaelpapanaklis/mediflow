...
import { DENTAL_LAB_SERVICES, type DentalLabServiceKey } from "@/lib/laboratorios/types";
...
/** Chip-toggle reutilizable para multi-select de servicios. */
function Chip({
  label,
  selected,
  onToggle,
...
export function LabRegistroForm() {
  ...
  const [services, setServices] = useState<DentalLabServiceKey[]>([]);

  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [success, setSuccess] = useState(false);

  function toggleService(key: DentalLabServiceKey) {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    if (!name.trim()) return setError("El nombre del laboratorio es requerido.");
    ...
    setLoading(true);
    try {
      const res = await fetch("/api/laboratorios/auth/register", {
        method: "POST",
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
      ...
      <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 16 }}>
        ...
        <FormField
          ...
          placeholder="Laboratorio Dental Central"
          value={name}
        ...
        <FormField
          ...
          type="email"
          autoComplete="email"
          placeholder="contacto@milaboratorio.com"
          value={email}
        ...
        <FormField label="Servicios que ofreces">
          <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
            {DENTAL_LAB_SERVICES.map(svc => (
              <Chip
                key={svc.key}
                label={svc.short}
                selected={services.includes(svc.key)}
