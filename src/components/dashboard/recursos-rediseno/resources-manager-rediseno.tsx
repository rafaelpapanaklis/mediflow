...
/**
 * Rediseño de Recursos — mismo CRUD que `resources/resources-manager.tsx`
 * (misma lógica, mismas mutaciones de `@/lib/agenda/mutations`, mismo tipo
 * `ResourceDTO`), solo que restylado con el lenguaje visual del menú de dos
 * niveles. Es una copia y no una variante de ese archivo a propósito: el
 * original lo sigue usando también `agenda-resources-modal.tsx` (variant
 * "modal") desde la Agenda, un vertical que no es este, y tocarlo habría
 * cambiado su aspecto sin que nadie de aquí lo pueda revisar. Esta copia
 * SOLO se monta para variant="page" con el interruptor `menu-dos-niveles`
 * encendido; el cajón de horarios (`ResourceScheduleDrawer`) se reutiliza
 * tal cual, sin fork — es un Dialog de Radix con su propio estilo y se abre
 * con un clic, no con la pantalla.
 */
...
import { ResourceScheduleDrawer } from "@/components/dashboard/resources/resource-schedule-drawer";
import { RaizRediseno } from "./raiz";
import styles from "./recursos-rediseno.module.css";
...
export interface ResourcesManagerRedisenoProps {
...
export function ResourcesManagerRediseno({
  initialResources,
  canEdit = true,
}: ResourcesManagerRedisenoProps) {
  const t = useT();
  ...
  return (
    <RaizRediseno className={styles.page}>
...
function ResourceAddFormRediseno({ onCancel, onSave }: ResourceAddFormProps) {
  const t = useT();
    ...
    >
      <div className={styles.addFormRow}>
        <input
          ...
          autoFocus
        />
        <label className={styles.addLabel} htmlFor="resource-add-kind-rediseno">
          {t("procurement.resourcesManager.typeLabel")}
        </label>
        <select
          id="resource-add-kind-rediseno"
          className={styles.kindSelect}
          value={kind}
