...
// El hilo y el composer de Sabina se pintan en `sabina-conversacion.tsx` (lo
// comparten la pantalla y el cajón lateral) con las clases de la pantalla: el
// mapa es el de `sabina-client.tsx`, que se lo baja por `clases`.
test("components/sabina/sabina-conversacion.tsx: toda clase pasa por `c` y el mapa de la pantalla la traduce", () => {
  const texto = leer("components/sabina/sabina-conversacion.tsx");
  const cliente = leer("app/dashboard/sabina/sabina-client.tsx");
  assert.ok(texto.includes('import styles from "@/app/dashboard/sabina/sabina.module.css"'), "perdió su módulo de siempre");
  assert.match(texto, /clases \?\? styles/, "sin `clases` (el cajón lateral) no vuelve a las de siempre");
  assert.ok(!/\bstyles\.[a-zA-Z]/.test(texto), "hay una clase que no pasa por `c` (se mezclarían las dos pieles)");
  // El bloque de props de un elemento, de `<Nombre` a su `/>` (las flechas `=>` de los props impiden cortar
  // en `>`). Con espacio detrás, para no confundirlo con la mención `<Nombre>` de un comentario.
  ...
  assert.ok(bloque(cliente, "SabinaConversacion").includes("clases={c}"), "la pantalla no le baja su juego de clases");
  assert.ok(bloque(cliente, "SabinaConversacion").includes("rediseno={rediseno}"), "la pantalla no le baja la bandera (tarjetas)");
  assert.ok(bloque(texto, "PropuestaCard").includes("rediseno={rediseno}"), "las tarjetas no reciben la bandera");

  const inicio = cliente.indexOf("const CLASES_REDISENO: Record<string, string> = {");
  const mapa = cliente.slice(inicio, cliente.indexOf("\n};", inicio));
  const traducidas = new Set([...mapa.matchAll(/^\s+([a-zA-Z0-9_]+):/gm)].map((m) => m[1]));
  const viejas = clasesDe(leer("app/dashboard/sabina/sabina.module.css"));
  const usadas = new Set(
    [...texto.matchAll(/\bc\.([a-zA-Z0-9_]+)/g)].map((m) => m[1]).filter((k) => viejas.has(k)),
  );
  assert.ok(usadas.size >= 20, `se esperaban decenas de clases, hay ${usadas.size}`);
...
test("los destinos y endpoints son los de siempre", () => {
  const destinos: Record<string, string[]> = {
    // Desde «Sabina en todas partes» las llamadas de Sabina viven en su almacén,
    // que comparten la pantalla y el cajón lateral; la pantalla ya no llama a nada.
    "components/sabina/almacen.ts": [
