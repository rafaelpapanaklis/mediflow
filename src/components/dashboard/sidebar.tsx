        ...
        {NAV.map(item => {
          ...
          return (
            <Link key={item.href} href={item.href} onClick={() => setOpen(false)}
              ...
              {item.label}
              {isWa && !active && <span className="ml-auto w-1.5 h-1.5 rounded-full bg-emerald-400 flex-shrink-0" />}
            </Link>
          );
        })}
      </nav>
      ...
      <div className="px-3 pb-4 border-t border-white/10 pt-3 space-y-1">
        <button onClick={toggle}
          ...
          <div className="flex items-center gap-2.5">
            ...
            <span className="text-xs font-semibold text-slate-400">{dark ? "Modo oscuro" : "Modo claro"}</span>
          </div>
