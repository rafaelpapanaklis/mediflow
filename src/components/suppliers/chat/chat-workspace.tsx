...
import type { SupplierChatSender } from "@/lib/suppliers/types";

// ── Endpoints (contrato de types.ts) ─────────────────────────────────────
const API_THREADS = "/api/supplier-chat";
const API_START = "/api/supplier-chat/start";
const messagesUrl = (threadId: string, after?: string | null) =>
  `/api/supplier-chat/${threadId}/messages${after ? `?after=${encodeURIComponent(after)}` : ""}`;

const THREADS_POLL_MS = 5000;
const MESSAGES_POLL_MS = 4000;

type Side = "CLINIC" | "SUPPLIER";
...
//    de los campos crudos del hilo (clinic/supplier, clinicUnread/...). ──
function counterpartName(t: ChatThread, side: Side): string {
  return side === "CLINIC" ? t.supplier?.businessName ?? "Proveedor" : t.clinic?.name ?? "Clínica";
}
function counterpartLogo(t: ChatThread, side: Side): string | null {
  return (side === "CLINIC" ? t.supplier?.logoUrl : t.clinic?.logoUrl) ?? null;
}
function unreadFor(t: ChatThread, side: Side): number {
  return side === "CLINIC" ? t.clinicUnread : t.supplierUnread;
}
...
export function ChatWorkspace({
  ...
  side: Side;
  /** Solo lado clínica: proveedor con el que abrir/crear el hilo al entrar. */
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (side === "CLINIC" && initialSupplierId) {
        await startThread(initialSupplierId, ctrl.signal);
      }
    })();
    return () => ctrl.abort();
      ...
      >
        <header className="border-b border-border px-4 py-3">
          ...
          <p className="text-xs text-muted-foreground">
            {side === "CLINIC" ? "Conversaciones con proveedores" : "Conversaciones con clínicas"}
          </p>
        </header>
