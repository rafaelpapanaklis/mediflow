 ...
 *   · un modelo desconocido, vacío o un alias se cobra al precio MÁS ALTO de la
 *     tabla: nunca revienta y nunca cobra cero;
...
const MAS_CARO = { input: 10, output: 50 }; // Fable 5 / 5.1
...
// ── 3. Modelo desconocido: el más caro, nunca revienta ni cobra cero ────────
for (const raro of ["", "   ", "sonnet", "claude-sonnet", "gpt-4o", "claude-opus-9", undefined as any, null as any]) {
  test(`modelo desconocido ${JSON.stringify(raro)} → precio más alto, > 0, sin lanzar`, async () => {
    const r = await billed(raro, M, M);
    assert.equal(r.micros, (MAS_CARO.input + MAS_CARO.output) * M);
    assert.ok(r.cents > 0);
  });
}

test("un modelo desconocido nunca sale más barato que ninguno conocido", async () => {
  const desconocido = await billed("modelo-que-no-existe", 10_000, 1_000, 500, 700);
  for (const model of Object.keys(LISTA)) {
...
test("cada modelo que pasan los llamadores está en la tabla (no cae en el más caro)", async () => {
...
test("un modelo desconocido también paga la caché al precio más alto", async () => {
  const r = await billed("modelo-nuevo", 0, 0, M, M);
  assert.equal(r.micros, (1 + 12.5) * M); // lectura de Fable 5 + escritura de Fable
});
...
test("una fila con id no canónico o de un modelo fuera de la tabla no pisa nada", async () => {
  ...
  assert.equal((await billed("constructor", M, 0)).micros, MAS_CARO.input * M);
  assert.equal((await billed("gpt-4o", M, 0)).micros, MAS_CARO.input * M);
});
...
test("un nombre heredado de Object ('constructor') no se cuela como precio", async () => {
  ...
  assert.equal(r.micros, MAS_CARO.input * M);
});
