 ...
 *
 * "network_bonus" NO sale de `resolveCommission`: lo escriben los BONOS POR
 * RED (@/lib/affiliates/network-bonus) para entrar por el mismo flujo de pago,
 * reportes y estado de cuenta que el resto. Vive en este tipo para que toda
 * pantalla que etiquete un `kind` tenga que contemplarlo.
 */
export type CommissionKind = "pct" | "recurring" | "onetime" | "network_bonus";
...
export function commissionKindLabel(kind: string | null | undefined): string {
  const k =
    kind === "recurring" || kind === "onetime" || kind === "network_bonus" ? kind : "pct";
