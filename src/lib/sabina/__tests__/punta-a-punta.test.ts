...
test("el modelo recibe las diez de consulta, las de agenda, pacientes y dinero, la de caja, las tres de clínico y la de lo que se escapa, con su esquema", async () => {
  estado.guion = () => contesta("Hola.");
  const res = await preguntar("hola");
  assert.equal(res.status, 200);

  const tools = estado.peticiones[0]?.tools ?? [];
  assert.deepEqual(tools.map((t: any) => t.name).sort(), [...LAS_DIEZ, ...LAS_NUEVAS, ...LAS_DE_CAJA, ...LAS_DE_CLINICO, ...LAS_DE_ESCAPE].sort());
