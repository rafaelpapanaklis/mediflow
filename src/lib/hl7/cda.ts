...
// la nota firmada y la adenda. Numéricos a propósito: un OID solo admite arcos
// numéricos.
const OID_MEDIFLOW_NOTE = "2.16.840.1.113883.3.7777.1.3";
const OID_MEDIFLOW_ADDENDUM = "2.16.840.1.113883.3.7777.1.4";
// El autor de una adenda es un usuario de DaleControl, igual que el autor del
// documento cuando no tiene cédula: va bajo la MISMA raíz de usuario, para que
// un receptor reconozca a la misma persona en los dos sitios.
const OID_MEDIFLOW_USER = UUID_DALECONTROL_USUARIO;
...
function appendAddendaSection(
  ...
  addenda.forEach((a, i) => {
    ...
    if (a.authorId) assigned.ele("id", { root: OID_MEDIFLOW_USER, extension: a.authorId, assigningAuthorityName: "DaleControl (usuario)" });
