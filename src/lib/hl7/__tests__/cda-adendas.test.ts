...
/**
 * LA SALIDA DE HOY, literal. Es lo que produce `buildCdaXml` en
 * `fix/dental-cda-esquema` (#241, commit cf0c231e: el CDA que ya valida contra
 * el XSD oficial y todavía no sabe nada de adendas) con la entrada
 * `ENTRADA_FIJA` de abajo, generada con TZ=UTC y pegada aquí a mano. Hasta la
 * integración con el #241 era la de `origin/main` (5aa46ed2); cambia en cuatro
 * sitios, los cuatro del #241 (OIDs de cédula y CLUES, la especialidad como
 * `<code>` antes de `<assignedPerson>`, y el `<code>` del diagnóstico).
 * Sirve de ancla del tercer requisito: "un paciente sin adendas produce un CDA
 * idéntico al de hoy". Si algo de este trabajo se cuela en el
...
<ClinicalDocument xmlns="urn:hl7-org:v3" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:mediflow="urn:mediflow:cda">
  ...
  <author>
    ...
    <assignedAuthor>
      <id root="2.16.840.1.113883.3.215.12.18" extension="12345678"/>
      <code nullFlavor="OTH">
        <originalText>Odontología</originalText>
      </code>
      <assignedPerson>
        <name>
          <given>Ana</given>
  ...
  <custodian>
    <assignedCustodian>
      <representedCustodianOrganization>
        <id root="2.16.840.1.113883.4.631" extension="DFSSA000001"/>
...
/**
 * DEFECTOS QUE YA VENÍAN DE ANTES. Esta rama nació sobre un generador que no
 * validaba contra el XSD de CDA R2 (la especialidad después de
 * `<assignedPerson>`, raíces `…7777.cedula`/`…7777.clues`/`…7777.curp` que no
 * son OIDs, y el `<observation>` de diagnósticos sin `<code>`), y los dejaba
 * FIJADOS aquí para que no apareciera ninguno nuevo y para que esta prueba
 * avisara el día que alguien los arreglara.
 *
 * Ese día llegó con la integración del #241 (fix/dental-cda-esquema): el CDA
 * valida contra el XSD oficial y la lista queda VACÍA. Se mantiene la
 * constante —y no se borra la comprobación— para que un defecto nuevo vuelva a
 * hacer saltar la prueba. La validación de verdad, contra el XSD y CON
 * adendas, vive en `cda-esquema.test.ts`.
 */
const DEFECTOS_PREVIOS: string[] = [];
...
test("H25-CDA · el XML está bien formado y su estructura es la que manda CDA R2", async () => {
  ...
  // Y el suelo del que se parte, dicho en voz alta: una nota y ningún defecto
  // (los cuatro que venían de antes los cerró el #241).
